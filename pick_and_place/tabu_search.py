...
class Taboo(object):
    ...
    def run(self, soluzione):
        '''
            finchè non incontro la condizione di fine:
                genero un intorno della soluzione
                pesco a caso una soluzione ammissibile

                se il goal raggiungo dalla nuova soluzione è minore della best
                Sbest = attuale soluzione
        '''

        # attuale soluzione migliore è quella che ci siamo salvati
        self.best = soluzione
        self.Sk = soluzione
        ...
        while (True) :
            ...
            # controllo se Sc è migliore di Sbest
            # se il numero di 5 contenuti in Sc è minore del numero di 5 contenuti in Sbest
            if Sc.count(5) < self.best.count(5):
                # se è migliore, sbest = sc e ricomincio il ciclo
                self.best = Sc
    ...
    def check(self) :
        # controllo se 
        return (self.fail > self.maxtentativi) || (self.k > self.times) || (self.notfound > self.maxnotfound) 
